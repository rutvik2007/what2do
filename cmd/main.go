...
func getChannelID(service *youtube.Service, forUsername string) (string, error) {
	...
	if numItems == 0 {
		return "", errors.New("the api does not work for this channel idk why")
	}
	if numItems != 1 {
		log.Fatalf("getChannelUploadID: searching for channel %s - Expected 1 Item, received %d", forUsername, numItems)
	}

	if response.Items[0].ContentDetails == nil || response.Items[0].ContentDetails.RelatedPlaylists == nil {
		return "", errors.New("content details are nil")
	}
	return response.Items[0].Id, nil
}

func getPlaylistVideos(service *youtube.Service, forPlaylist string) []what2cook.Video {
